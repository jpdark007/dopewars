...
public class TradeWars extends javax.swing.JFrame {
	...
	/**
	 * starts monitoring for shortcut codes "m" will focus the "Move" box "b"
	 * will focus the "Buy" table "s" will focus the "sell" table
	 */
	...
	private void initComponents() {
		...
		tradeQuantity.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(
	...
	private void makeTradeActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_makeTradeActionPerformed
		...
		try {
			...
			if (trade.getMode() == Trade.Mode.BUY) {
				logMsg = String.format("You bought %d \u00D7 %s at %s!", quantity, trade.getProduct().getName(),
				...
				logMsg = String.format("You sold %d \u00D7 %s for %s, making %s!", quantity,
	...
	public void handleGameEvents() {
		...
		for (Event e : game.getEvents()) {
			if (e.requiresInput()) {
				response = JOptionPane.showConfirmDialog(this, e.getMessage(), "Game Event", JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE, IconFactory.getDialogIcon(JOptionPane.QUESTION_MESSAGE));
